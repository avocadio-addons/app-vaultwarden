...
# Read with awk rather than `sed ... | head -n1`: under `pipefail` head can
# close the pipe before sed is done and the SIGPIPE takes the whole script
# down with exit 141. awk stops on its own after the first match.
pinned="$(sed -n \
    's|^FROM "vaultwarden/server:\(.*\)" AS vaultwarden$|\1|p' "${dockerfile}")"
shipped="$(awk '/^- .* Update Vaultwarden to [0-9][0-9.]*$/ {
    sub(/^.* to /, ""); print; exit }' "${changelog}")"
...
# The README states the shipped version once, in the fork notice. A pattern
# that stops matching only means the README drifts, so warn, do not fail the
# release over prose.
before="$(cat "${readme}")"
sed -i "s|\*\*Vaultwarden [0-9][0-9.]*\*\*|**Vaultwarden ${target}**|g" \
    "${readme}"
